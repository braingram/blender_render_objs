...
def obj_image_load(imagepath, DIR, IMAGE_SEARCH):
    if b'_' in imagepath:
        ...
        if image:
            return image
    print('BJG:'+str(imagepath))
    image = load_image(imagepath, DIR)
    if image:
        return image
    
    # failed to find image, look in local directory
    #print("Imagepath %s" % imagepath)
    #print(" type: %s" % str(type(imagepath)))
    #print("DIR %s" % DIR)
    #print(" type: %s" % str(type(DIR)))
    image = load_image(DIR + b'/' + imagepath.split(b'\\')[-1], DIR)
    #image = load_image(bytes(str(imagepath.split('\\')[-1],encoding='utf8'),'utf8'), DIR)
    if image:
        return image
...
def create_materials(filepath, material_libs, unique_materials, unique_material_images, IMAGE_SEARCH):
    ...
    def load_material_image(blender_material, context_material_name, imagepath, type):
        ...
        if type == 'Kd':
            ...
            mtex.use_map_color_diffuse = False # 3ds hack to fix bump maps
            ...
            blender_material.alpha = 0.0
            mtex.use_map_color_diffuse = False # 3ds hack to fix alpha maps
            ...
            for line in mtl:  # .readlines():
                ...
                if not line or line.startswith(b'#'):
                    ...
                    if line_lower.startswith(b'ka'):
                        ...
                        context_material.alpha = 1. - float(line_split[1])
...
#             ob.setSize(scale, scale, scale)
    ...
    xscale = abs(1. / (xmax - xmin))
    yscale = abs(1. / (ymax - ymin))
    zscale = abs(1. / (zmax - zmin))
    
    scale = min(xscale,yscale,zscale)
    
    #recenter
    dx = (xmax + xmin) / 2. * scale
    dy = (ymax + ymin) / 2. * scale
    dz = (zmax + zmin) / 2. * scale
